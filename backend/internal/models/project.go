...
func (m *ProjectModel) GetByID(ctx context.Context, id uuid.UUID) (*Project, error) {
	...
	err := m.db.QueryRowContext(ctx,
		...
		id,
	).Scan(&project.ID, &project.Name, &description, &project.OwnerID, &project.CreatedAt, &project.UpdatedAt)
...
func (m *ProjectModel) ListForUser(ctx context.Context, userID uuid.UUID) ([]Project, error) {
	...
	for rows.Next() {
		var p Project
		var description sql.NullString
		if err := rows.Scan(&p.ID, &p.Name, &description, &p.OwnerID, &p.CreatedAt, &p.UpdatedAt); err != nil {
			return nil, err
		}
		if description.Valid {
...
func (m *ProjectModel) Update(ctx context.Context, id uuid.UUID, name, description *string) (*Project, error) {
	...
	err := m.db.QueryRowContext(ctx,
		`UPDATE projects SET name = COALESCE($1, name), description = COALESCE($2, description), updated_at = NOW() 
		 WHERE id = $3 RETURNING id, name, description, owner_id, created_at, updated_at`,
		name, description, id,
	).Scan(&project.ID, &project.Name, &desc, &project.OwnerID, &project.CreatedAt, &project.UpdatedAt)
	if err != nil {
		return nil, err
	}
	if desc.Valid {
