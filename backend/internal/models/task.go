...
func (m *TaskModel) Create(ctx context.Context, title string, description *string, status, priority string, projectID, assigneeID uuid.UUID, dueDate *string) (*Task, error) {
	var task Task
	var desc, assignee, due sql.NullString
	...
	).Scan(&task.ID, &task.Title, &desc, &task.Status, &task.Priority, &task.ProjectID, &assignee, &due, &task.CreatedAt, &task.UpdatedAt)
	if err != nil {
		return nil, err
	}
	if desc.Valid {
...
func (m *TaskModel) GetByProjectID(ctx context.Context, projectID uuid.UUID) ([]Task, error) {
	...
	for rows.Next() {
		var t Task
		var desc, assignee, due sql.NullString
		if err := rows.Scan(&t.ID, &t.Title, &desc, &t.Status, &t.Priority, &t.ProjectID, &assignee, &due, &t.CreatedAt, &t.UpdatedAt); err != nil {
			return nil, err
		}
		if desc.Valid {
...
func (m *TaskModel) GetByProjectIDWithFilters(ctx context.Context, projectID uuid.UUID, status, assigneeID string) ([]Task, error) {
	...
	if assigneeID != "" {
		argNum++
		query += " AND assignee_id = $" + string(rune('0'+argNum))
...
func (m *TaskModel) GetCreatorID(ctx context.Context, taskID uuid.UUID) (uuid.UUID, error) {
	var assigneeID uuid.UUID
	err := m.db.QueryRowContext(ctx, "SELECT assignee_id FROM tasks WHERE id = $1", taskID).Scan(&assigneeID)
